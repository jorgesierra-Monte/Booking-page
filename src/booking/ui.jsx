...
export function Button({ use = 'primary', className, children, ...rest }) {
  const base =
    'inline-flex items-center justify-center min-h-component-mediumButtons px-300 rounded-rounded typography-label-emphasis-default transition disabled:opacity-50 disabled:cursor-not-allowed'
  const uses = {
    primary: 'bg-action-primary-surface text-action-primary-text hover:bg-action-primary-hover',
    secondary:
      'bg-action-secondary-surface text-action-secondary-text ring-1 ring-action-secondary-border hover:bg-action-secondary-hover',
    'link-secondary':
      'min-h-0 px-0 underline underline-offset-4 text-text-default hover:text-text-muted bg-transparent',
  }
  return (
...
const fieldInput =
  'h-[52px] w-full rounded-small bg-transparent px-250 pb-[6px] pt-[18px] typography-label-default text-text-default outline-none placeholder:text-text-muted'
...
export function SelectField({ label, options = [], required, optional }) {
  const [focused, setFocused] = useState(false)
  const [value, setValue] = useState('')
  const active = focused || value !== ''
  return (
    <div className={fieldShell}>
      ...
      <select
        ...
        onFocus={() => setFocused(true)}
        onBlur={() => setFocused(false)}
        className={cx(fieldInput, 'cursor-pointer appearance-none pr-450', !active && 'text-transparent')}
      >
        <option value="" disabled hidden />
        {options.map(o => (
          <option key={o} value={o} className="text-text-default">
            {o}
          </option>
        ))}
      </select>
      <svg
        viewBox="0 0 20 20"
        className="pointer-events-none absolute right-250 top-1/2 h-4 w-4 -translate-y-1/2 text-text-default"
        fill="none"
        aria-hidden="true"
      >
